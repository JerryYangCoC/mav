    ...
    <div>
        <form @submit.prevent="onClick()">
            <div style="display: grid; grid-template-columns: 160px auto auto; align-items: center; justify-content: start; justify-items: end;">
                <span><span class="ask-red">*</span>精算抵用券：</span>
                <!-- <input type="text" style="width: 300px;" v-model="value.CouponsList" required /> -->
                <div style="width: 300px; height: 40px; border: 1px solid #ccc; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; line-height: 40px; padding-left: 8px;" required>
                    <span v-for="(item, index) in value.CouponsList" :key="index" style="padding-right: 4px;">{{ index > 0 ? ' | ' + item?.CouponName : item?.CouponName }}</span>
                </div>
    ...
    <Dialog v-model:visible="queryView" modal :show-header="false" :style="{ width: '90vw' }">
        <div class="box-Cop">
            <div style="text-align: left; display: flex; margin: 8px 0;">
                <input type="text" style="width: 200px;" v-model="query.CopCode" placeholder="編號" @change="onSearch()" />
                <input type="text" style="width: 200px;" v-model="query.CopDesc" placeholder="名稱" @change="onSearch()" />
                ...
                <div style="margin-left: 10px;">
                    ...
                    <select style="width: 100px;" v-model="query.MoneyType" @change="onSearch(); selectedProduct = []">
                        <option value="0">金額</option>
                        <option value="1">百分比</option>
                    </select>
                </div>
                
                <div style="margin-left: 10px;">
                    <span><span class="ask-red">*</span>券別：</span>
                    <select style="width: 100px;" v-model="query.CopType" @change="onSearch(); selectedProduct = []">
                        <option value="0">全部</option>
                        <option value="3">金卡禮</option>
                        <option value="4">金卡續卡禮</option>
                        <option value="8">活動特給</option>
                    </select>
                </div>
                ...
                <div style="min-width: 40vw; border: 1px solid #ccc;" class="p-fileupload p-fileupload-advanced p-component">
                    <div class="p-fileupload-file" v-for="(item, index) in selectedProduct" :key="index">
                        ...
                        <div class="p-fileupload-file-actions">
                            <button style="width: 24px;" class="p-button p-component p-button-icon-only p-button-danger p-button-rounded p-button-text p-fileupload-file-remove" type="button" @click="selectedProduct.splice(index, 1)" outlined rounded  severity="danger">
...
export default class JourneyTmp202 extends Vue {
    dd!: JourneyModel;
    ...
    created(): void {
        this.selectedProduct = this.value.CouponsList?.map((ver: CouponModel) => {
            return {
    ...
    onClickProduct(): void {
        this.value.CouponsList = this.selectedProduct.map((ver: CouponModel) => {
            return {
     ...
     */
    onSearch(): void {
        store.dispatch('getListJourneyCouponM', this.query)
    }

    copList(): any {
        return store.state.journey.journeyCouponMList
    }
}
</script>
